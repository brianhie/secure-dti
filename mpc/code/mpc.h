...
public:
  ...
  template<class T>
  void BeaverMult(T& ab, Vec<T>& ar, Vec<T>& am, Vec<T>& br, Vec<T>& bm, int fid = 0) {
    if (pid == 0) {
      ab += am * bm;
    } else {
      ab += ar * bm;
  ...
  /**
   * Input: a is secret shared data, am is empty.
   * Output: a is revealed but masked data, am has the secret shared mask.
   */
  template<class T>
  ...
  void BeaverPartition(Mat<T>& ar, Mat<T>& am, Mat<T>& a, int fid = 0) {
    if (debug) tcout() << "BeaverPartition: " << a.NumRows() << ", " << a.NumCols() << endl;
